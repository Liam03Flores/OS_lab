...
static void print_board(void) {
    ...
    if (!game_over)
        printf("\033[33m\n  Numero de pregunta (Ctrl+c  = salir): \033[0m");
    fflush(stdout);
}
...
int main(void) {
    ...
    pid_t pid_I = fork();
    if(pid_I == 0){
        printf("\033[33;1mInstrucciones del juego:\033[0m\n");
        printf("• Escribe el número de la pregunta (1 - 6) que deseas responder.\n");
        printf("• Después escribe la palabra en minusculas o mayusculas (no es necesario que incluyas acentos).\n");
        printf("• Si aciertas, la palabra se revela en el tablero.\n");
        printf("• Si fallas, la palabra puede cambiar con el tiempo.\n");
        printf("• Una palabra cmbiara cada %d segundos aleatoriamente.\n", CHANGE_INTERVAL);
        printf("• Oprime ctrl + C en cualquier momento para salir.\n\n");
        exit(0);
    }
    ...
    /*SIGINT para salir del programa*/
